# WordMaster — Technical Specification (ТЗ)
...
* WPF приложение (.NET 7/8)
* Разделение проектов: UI, Domain, Data
* Entity Framework Core для доступа к БД
* Data binding, MVVM-подход
* Локальная СУБД: SQLite (рекомендовано) или LocalDB
* CRUD для основных сущностей
* Тестовое окно с подсчётом результатов
* Таблица платежей (Payments) и окно оплаты (симуляция)

**Рекомендованные:**

* DI контейнер (Microsoft.Extensions.DependencyInjection)
* AutoMapper для маппинга DTO ↔ Entity
* FluentValidation для валидации форм
